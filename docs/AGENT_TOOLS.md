...
| `parse_single_crystal_data` | Parse single-crystal reflections | Parse single-crystal integrated intensities — a FullProf .int (h k l I σ) or a SHELX HKLF4 .hkl — into a SingleCrystalDataset. The entry point for refine_joint_single_crystal; parse the nuclear and magnetic files with two calls. |
