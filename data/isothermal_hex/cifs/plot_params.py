...
def save_scientific_csv(filename, headers, data_columns):
    """
    Saves multiple numpy arrays as columns, automatically handling 
    and broadcasting scalars to match array lengths.
    """
    if len(headers) != len(data_columns):
        raise ValueError(f"Mismatch: {len(headers)} headers vs {len(data_columns)} columns.")

    # 1. Identify the target length from the actual arrays in the list
    # We filter out scalars to find the true row count
    lengths = [len(np.atleast_1d(arr)) for arr in data_columns if np.ndim(arr) > 0]
    
    if not lengths:
        target_len = 1 # Case where all inputs are scalars
    else:
        target_len = max(lengths)
        if len(set(lengths)) > 1:
            raise ValueError(f"Array length mismatch among sequences: {lengths}")

    # 2. Process columns: Broadcast scalars to target_len
    processed_columns = []
    for arr in data_columns:
        arr_np = np.atleast_1d(arr)
        if len(arr_np) == 1 and target_len > 1:
            # Broadcast scalar to match the rest of the data
            processed_columns.append(np.full(target_len, arr_np[0]))
        else:
            processed_columns.append(arr_np)

    # 3. Create DataFrame and Save
    df = pd.DataFrame(dict(zip(headers, processed_columns)))
    df.to_csv(filename, index=False, float_format='%.8e')
    print(f"Successfully saved {filename} with {target_len} rows.")
...
#ax.errorbar(T_tmp[::-1], 10000*(a_all-a_all[-1])/a_all[-1],1E-0*a_all/a_all[-1],
ax.errorbar(T_all, 1000*(a_all-a_all[-1])/a_all[-1],1E-1*a_all/a_all[-1],
		lw=0.0,
		marker='o',ms=5.0,mew=1.0,mfc='w',
		capsize=2,elinewidth=1.0,
		label='a=b',
		color=col[0],alpha=1.0)
'''ax.errorbar(T_all, b_all/b_all[-1],1E-4*b_all/b_all[-1],
		lw=0.0,
		marker='o',ms=ms,mew=0.8,mfc='w',
		capsize=2,elinewidth=0.5,
		label='b',
		color=col[5],alpha=1.0)'''
#ax.errorbar(T_tmp[::-1], 10000*(c_all-c_all[-1])/c_all[-1],1E-0*c_all/c_all[-1],
ax.errorbar(T_all, 1000*(c_all-c_all[-1])/c_all[-1],1E-1*c_all/c_all[-1],
		lw=0.0,
		marker='o',ms=5.0,mew=1.0,mfc='w',
		capsize=2,elinewidth=1.0,
		label='c',
...
ax.set_ylabel(r'$\mathrm{\Delta l /l_{0}}$ ($\mathrm{\times 10^{-3}}$)',fontsize=10) # latt

# Save data for NC data availability
# save_scientific_csv(
# 	filename="Figure_2b_hexlatt.csv",
# 	headers=["T", "a", "b"],
# 	data_columns=[T_all, a_all, b_all]
# )
...
#fig.subplots_adjust(left = 0.32,right = 0.97, bottom = 0.18, top = 0.98,wspace =0.02,hspace = 0.30) # potrait
#fig.set_size_inches((0.95*3.375/2,3.375*3/4))

#fig.subplots_adjust(left = 0.19,right = 0.98, bottom = 0.18, top = 0.98,wspace =0.02,hspace = 0.30)
#fig.set_size_inches((3.375,3.375*3/4))

fig.subplots_adjust(left = 0.28,right = 0.98, bottom = 0.18, top = 0.98,wspace =0.02,hspace = 0.30) # SciAdv
fig.set_size_inches((3.54/2,3.375*3/4))
