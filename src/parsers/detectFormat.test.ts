...
describe("detectDataFormat — data type", () => {
  ...
  it("classifies a .gr file as pdf by extension (high confidence)", () => {
    const d = detectDataFormat({ text: "0.01 -0.002\n0.02 -0.004\n0.03 -0.006", filename: "Ni.gr" });
    expect(d.dataType).toBe("pdf");
    expect(d.source).toBe("filename");
    expect(d.confidence).toBe("high");
  });
  it("classifies a PDFgetX3 header as pdf regardless of extension", () => {
    const text = "# generated by diffpy.pdfgetx3\n# qmax = 28.0\n#### start data\n0.01 -0.002\n0.02 -0.004";
    const d = detectDataFormat({ text, filename: "sample.dat" });
    expect(d.dataType).toBe("pdf");
    expect(d.source).toBe("header");
  });
  it("a pdf override wins over content detection", () => {
    const d = detectDataFormat({ text: "1.0 0.5\n1.1 0.4\n1.2 0.3", filename: "x.dat", override: { dataType: "pdf" } });
    expect(d.dataType).toBe("pdf");
