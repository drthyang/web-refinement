...
export const TOOL_REGISTRY: readonly ToolDefinition[] = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      extraPhases: anyArr.optional().describe("Additional crystallographic phases (multi-phase)"),
      fitRange: z.object({ min: z.number().optional(), max: z.number().optional() }).optional(),
      nSteps: z.number().int().min(1).max(20000).describe("Ensemble steps THIS call (each = one proposal per walker)"),
      nWalkers: z.number().int().min(4).max(256).optional().describe("Even; default max(2·nFree+2, 12)"),
      burnIn: z.number().int().min(0).optional().describe("Steps (from chain start) excluded from summaries; default half"),
