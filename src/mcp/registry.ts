...
export const TOOL_REGISTRY: readonly ToolDefinition[] = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      extraPhases: anyArr.optional().describe("Additional crystallographic phases (multi-phase G(r) sum)"),
    },
    handler: tools.build_pdf_model,
  },
  {
    name: "refine_pdf",
  ...
  },
  {
    ...
    title: "Sample Bayesian posterior (MCMC)",
    description: "Draw Bayesian posterior samples over the FREED parameters of a PDF fit with an affine-invariant ensemble MCMC sampler — the full parameter distribution where refine_pdf gives a point estimate. Reports per-parameter median / 68% / 95% credible intervals, convergence diagnostics (R-hat, ESS, acceptance), sample correlations, and — when `linearizedEsd` from a prior refine_pdf is supplied — `esdRatio` (posterior std / least-squares esd: ≈1 validates the esds, ≫1 means they were overconfident from correlation or non-Gaussianity). The default `marginalized` noise model integrates out the unknown G(r) error scale (uniform weights are not honest sigmas), so widths are meaningful. Run refine_pdf FIRST and seed `parameters` at the converged values. Bounded per call: if `converged` is false, pass the returned `resume` token to a follow-up call to continue the same chain.",
    inputSchema: {
      structure: anyObj, pattern: anyObj,
      parameters: anyArr.describe("RefinementParameter[] seeded at the refine_pdf optimum (fixed:true excludes from sampling)"),
      bindings: anyArr,
      restraints: anyArr.optional(),
      extraPhases: anyArr.optional().describe("Additional crystallographic phases (multi-phase)"),
      fitRange: z.object({ min: z.number().optional(), max: z.number().optional() }).optional(),
      nSteps: z.number().int().min(1).max(20000).describe("Steps THIS call (ensemble: one proposal per walker each; nuts: kept draws per chain)"),
      sampler: z.enum(["ensemble", "nuts"]).optional().describe("Default ensemble (gradient-free, pool-parallel). nuts = gradient-based NUTS via the analytic gradChi2 (single-phase PDF only): far better mixing per evaluation, LM esds seed the mass matrix — pass linearizedEsd"),
      nWalkers: z.number().int().min(4).max(256).optional().describe("ensemble only; even; default max(2·nFree+2, 12)"),
      burnIn: z.number().int().min(0).optional().describe("ensemble only: steps (from chain start) excluded from summaries; default half"),
      thin: z.number().int().min(1).optional(),
      ...
      })).optional().describe("Priors by parameter id; default uniform within bounds"),
      linearizedEsd: z.record(z.string(), z.number()).optional().describe("esds from refine_pdf, enables esdRatio"),
  ...
  {
    name: "compute_partial_pdf",
    title: "Partial PDF decomposition",
    description: "Decompose the calculated G(r) for interpretation — which chemistry (element pair) or which phase makes which peak. Single phase → Faber–Ziman element-pair partials; with extraPhases → per-phase contributions. The curves sum exactly to the total calc.",
    inputSchema: {
      structure: anyObj, pattern: anyObj,
      parameters: anyArr, bindings: anyArr,
      extraPhases: anyArr.optional(),
    },
    handler: tools.compute_partial_pdf,
  },
  {
    name: "calibrate_qdamp",
    title: "Calibrate Qdamp/Qbroad from a standard",
    description: "Calibrate the instrument resolution envelope from a measured STANDARD (Ni / Si / LaB₆ — a sample with a known structure): frees only the PDF scale + Qdamp + Qbroad, holds the certified structure, and returns the calibrated constants (with esds) to carry into sample fits as fixed values.",
    inputSchema: {
      structure: anyObj.describe("The standard's certified StructureModel"),
      pattern: anyObj.describe("The standard's measured PdfPattern"),
      fitRange: z.object({ min: z.number().optional(), max: z.number().optional() }).optional(),
      maxIterations: z.number().int().min(1).max(200).optional(),
    },
