...
export function PdfWorkbench({ structure, pattern, extraPhases = [], ownStructure = false, client, step = 0, onStep, exportsRef, onLoadData, onLoadCif, onAddPhase, onRemovePhase, presetValues, presetFitRange }: {
  ...
  exportCifRef.current = (): void => {
    ...
    for (const phase of phases) {
      // A refined spin model makes the output an mCIF (as on the powder page):
      // a plain CIF cannot carry moments, so exporting one would silently drop
      // the magnetic half of the refinement.
      if (spinFit && phase.structure === fitStructure) {
        downloadText(
          `${phase.structure.name || phase.id}_mpdf.mcif`,
          magneticStructureToMcif(phase.structure, refinedMagnetic ?? spinFit.magnetic, { params: withEsd, bindings: spec.bindings, refinement: meta }),
          "chemical/x-cif",
        );
        continue;
      }
